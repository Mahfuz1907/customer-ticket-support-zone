...
function TaskStatuses({inProgress, setInProgress, isResolved, setIsResolved, updatedTickets, setUpdatedTickets}:TaskStatusTasksProperties) {
    return (
        <div className='flex flex-col justify-center items-start gap-4 w-full'>
            <h1 className='text-black font-bold text-lg'>Task Status</h1>
            {
                ...
                {
                    inProgress.map((progressCard) => <StatusCard key={progressCard.id} progressCard={progressCard} isResolved={isResolved} setIsResolved={setIsResolved} inProgress={inProgress} setInProgress={setInProgress} updatedTickets={updatedTickets} setUpdatedTickets={setUpdatedTickets} />)
                }
            </div>
            }
        </div>
