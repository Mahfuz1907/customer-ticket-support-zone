...
function CustomerTickets({tickets, inProgress, setInProgress}: CustomerTicketsProperties) {
    return (
        <div className='flex flex-col justify-between items-start w-full gap-4'>
            ...
            <div className='grid grid-cols-2 justify-between items-start gap-7'>
                {
                    tickets.map((ticket) => <Ticket key={ticket.id} ticket={ticket} inProgress={inProgress} setInProgress={setInProgress} />)
                }
            </div>
        </div>
    );
